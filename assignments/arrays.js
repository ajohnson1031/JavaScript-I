...
let lastCar = 0; /* Why is this here?*/
...
for (let year of carYears) {
  year < 2000 ? oldCars.push(year) : null;
}
console.log(oldCars);
...
for (let i = 0; i < inventory.length; i++) {
  inventory[i].car_make === "BMW" || inventory[i].car_make === "Audi"
...
inventory.forEach(element => {
  element.car_model === "Accord"
    ? accord.push(`${element.car_year}, ${element.car_make}`)
    : null;
});
...
//Filter Experimentation II

let noobs = [];
const youngun = inventory.filter(elem => {
  return elem.car_year >= 2003;
});

for (car of youngun) {
  noobs.push(car.car_year);
}

noobs.sort();

console.log(`Fresh off the line!!! - ${noobs}`);
